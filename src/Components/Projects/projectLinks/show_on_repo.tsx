...
export function ProjectRepoToggle({ project }: ProjectRepoToggleProps) {
  ...
  async function handleToggle() {
    ...
    try {
      const updatedProjects = await updateBackendData<any>(
        ...
        { field: "show_on_cv", newValue: newValue, projectId: `${project.project_id}` },
        "PUT"
      );
  ...
  return (
    <label className="inline-flex items-center gap-3 cursor-pointer select-none group mt-6 pt-4 border-t border-slate-800/60">
      {/* Hidden native checkbox to handle input mechanics */}
      <input
        type="checkbox"
        checked={!!project.show_on_cv}
      ...
      <span className="text-sm font-medium text-slate-300 group-hover:text-slate-100 transition-colors">
        {isUpdating ? "Saving..." : "Show on Repo"}
      </span>
    </label>
  );
}
